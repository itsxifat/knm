  ...
  return (
    <div className="min-h-screen bg-white font-body text-[#121212] selection:bg-[#C5A059] selection:text-white">
      ...
      <div className="pt-10 pb-5 px-6 text-center bg-white relative z-10">
        {/* ✅ OPTIMIZED: Native CSS slide up fade in */}
      ...
      <div className="z-40 bg-white/90 backdrop-blur-md border-b border-[#F0F0F0] transition-all duration-300">
        <div className="max-w-[1920px] mx-auto">
             ...
             {/* Expanded Filter Panel (Native CSS Transitions) */}
             <div 
                className={`overflow-hidden bg-[#faf9f6] border-t border-[#F0F0F0] transition-all duration-500 ease-in-out ${showFilters ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0 border-t-0'}`}
             >
      ...
      <div className="relative min-h-[60vh] max-w-[1920px] mx-auto px-4 md:px-8 pb-32 pt-12">
          ...
          {/* Filtering Overlay (Native CSS) */}
          <div 
             className={`absolute inset-0 z-20 bg-white/60 backdrop-blur-[2px] transition-all duration-300 pointer-events-none ${isFiltering ? 'opacity-100' : 'opacity-0'}`} 
          />
          ...
          {(!data.sections?.some(s => s.products?.length > 0)) && (!data.mainProducts?.length) && (
             <div className="flex flex-col items-center justify-center py-32 m-4 rounded-sm border border-dashed border-[#E5E5E5] animate-in fade-in duration-500">
...
function ProductGrid({ title, products, slug }) {
  return (
    <div className="mb-24 last:mb-0 animate-in fade-in slide-in-from-bottom-4 duration-700 fill-mode-both">
