...
export default function SignupPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-white font-body px-4 relative overflow-hidden selection:bg-[#C5A059] selection:text-white">
      ...
      >
        ...
        <div className="text-center mb-10 flex flex-col items-center">
          <motion.div variants={itemVariants} className="relative w-56 h-28 mb-6">
             <Image src="/logo.png" alt="KNM" fill className="object-contain" priority />
          </motion.div>
          <motion.span variants={itemVariants} className="font-bold text-[10px] uppercase tracking-[0.3em] text-[#C5A059] mb-2 block">
            Membership
          </motion.span>
          <motion.h1 variants={itemVariants} className="text-3xl md:text-4xl font-heading font-normal text-[#121212] uppercase tracking-tight">
            Create Account
          </motion.h1>
        </div>
        ...
        <AnimatePresence mode="wait">
          {error && (
            ...
            >
              <div className="bg-[#121212] border-l-4 border-red-500 text-white px-5 py-4 text-xs font-bold rounded-sm flex items-center gap-3">
        ...
        <form action={handleSubmit} className="space-y-6">
          ...
          <motion.div variants={itemVariants} className="relative group">
            <User className={`absolute left-0 top-1/2 -translate-y-1/2 transition-colors duration-300 ${focusedField === 'name' ? 'text-[#C5A059]' : 'text-[#8C8279]'}`} size={18} />
            <input 
          ...
          {/* Email Field */}
          <motion.div variants={itemVariants} className="relative group">
            <Mail className={`absolute left-0 top-1/2 -translate-y-1/2 transition-colors duration-300 ${focusedField === 'email' ? 'text-[#C5A059]' : 'text-[#8C8279]'}`} size={18} />
            <input 
          ...
          {/* Phone Field */}
          <motion.div variants={itemVariants} className="relative group">
            <Phone className={`absolute left-0 top-1/2 -translate-y-1/2 transition-colors duration-300 ${focusedField === 'phone' ? 'text-[#C5A059]' : 'text-[#8C8279]'}`} size={18} />
            <input 
          ...
          {/* Password Field */}
          <motion.div variants={itemVariants} className="relative group">
            <Lock className={`absolute left-0 top-1/2 -translate-y-1/2 transition-colors duration-300 ${focusedField === 'password' ? 'text-[#C5A059]' : 'text-[#8C8279]'}`} size={18} />
            <input 
          ...
          <motion.div variants={itemVariants} className="pt-4">
            <button 
              disabled={loading} 
              className="w-full bg-[#C5A059] text-white h-14 font-bold uppercase text-xs tracking-[0.25em] hover:bg-[#121212] disabled:opacity-70 flex justify-center items-center gap-3 transition-all duration-300 shadow-lg hover:shadow-xl active:scale-[0.98] group"
            >
        ...
        <motion.div variants={itemVariants} className="text-center mt-10 border-t border-[#C5A059]/10 pt-6">
          <p className="text-xs text-[#8C8279] font-medium">
            Already a member? <Link href="/login" className="text-[#121212] font-bold uppercase tracking-widest border-b border-[#121212] hover:border-[#C5A059] hover:text-[#C5A059] transition-all pb-0.5 ml-1">Log In</Link>
          </p>
        </motion.div>
