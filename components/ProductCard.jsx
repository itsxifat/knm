        ...
        <div className="relative w-full aspect-[3/4] overflow-hidden bg-[#F9F6F0] mb-4 transform-gpu border border-transparent group-hover:border-[#C5A059]/20 transition-colors duration-500">
            ...
            {displayTag && (
                <div className="absolute top-0 left-0 p-3 z-10">
                    <span className={`backdrop-blur-md px-3 py-1.5 text-[9px] md:text-[10px] font-bold uppercase tracking-widest shadow-sm ${displayTag === 'SALE' ? 'bg-[#C5A059] text-white' : 'bg-white/95 text-[#C5A059]'}`}>
                        {displayTag}
                    </span>
                </div>
            )}
            ...
            <AnimatePresence>
                {showSizes && (
                    <motion.div 
                        ...
                        className="absolute inset-x-0 bottom-0 p-4 bg-white/95 backdrop-blur-xl border-t border-[#C5A059]/20 z-30 flex flex-col items-center gap-3"
                        onClick={(e) => { e.preventDefault(); e.stopPropagation(); }} 
                    >
                          <div className="flex justify-between items-center w-full mb-1">
                             <span className="text-[10px] font-bold uppercase text-[#57534E] tracking-widest">Select Size</span>
                             <button onClick={(e) => { e.stopPropagation(); setShowSizes(false); }} className="text-[#8C8279] hover:text-[#C5A059]">
                                 <X size={14} />
                             </button>
                          </div>
            ...
            {!showSizes && (
                <div className="absolute bottom-4 right-4 z-20 md:translate-y-4 md:opacity-0 md:group-hover:translate-y-0 md:group-hover:opacity-100 transition-all duration-500 ease-out">
                    <button 
                        ...
                        className={`bg-white text-[#121212] w-10 h-10 flex items-center justify-center hover:bg-[#C5A059] hover:text-white transition-colors shadow-xl border border-gray-100
