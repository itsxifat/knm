...
import { recordActiveSession } from '@/actions/analytics'; 
...
// --- MOBILE MENU DRAWER (Pure CSS Transitions) ---
const MobileMenu = ({ isOpen, onClose, navData, session }) => {
  ...
  return (
    <div className={`fixed inset-0 z-[150] lg:hidden transition-all duration-300 ${isOpen ? 'visible' : 'invisible pointer-events-none'}`}>
      {/* Background Overlay */}
      <div 
        onClick={onClose}
        className={`absolute inset-0 bg-[#121212]/40 backdrop-blur-sm transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0'}`}
      />
      
      {/* Sliding Drawer */}
      <div 
        className={`absolute top-0 left-0 h-full w-[85%] max-w-[340px] bg-[#F9F6F0] shadow-2xl overflow-y-auto flex flex-col border-r border-[#C5A059]/20 transition-transform duration-500 ease-out ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        {/* Header */}
        ...
        <div className="px-6 py-4 bg-white/50 relative z-20 shrink-0">
            ...
            {searchResults.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-[#C5A059]/20 shadow-xl rounded-sm overflow-hidden z-[130] max-h-[50vh] overflow-y-auto custom-scrollbar">
                  <div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-2 space-y-4">
          {navData?.links?.map((link) => (
            <div key={link.label} className="border-b border-[#C5A059]/10 pb-3 last:border-0">
              ...
              <div className={`overflow-hidden transition-all duration-300 ease-in-out ${activeSub === link.label ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
    ...
    <>
      ...
      >
        <div className="w-full max-w-[1920px] mx-auto px-6 md:px-12">
          ...
          {/* TOP ROW */}
          <div className="flex justify-between items-center h-14 md:h-[70px] relative z-[101]">
              ...
              <div className="hidden lg:flex items-center relative" ref={searchContainerRef}>
                  ...
                  >
                    ...
                    {!searchOpen ? (
                        ...
                        <form onSubmit={handleSearchSubmit} className="flex-1 ml-3 flex items-center min-w-0">
                           <input 
                             ref={searchInputRef}
                             type="text" 
                             value={searchQuery}
                             ...
                             placeholder="Search..." 
                             className="w-full bg-transparent text-sm font-body text-black font-medium focus:outline-none placeholder:text-gray-400 min-w-0"
                           />
                           {searchQuery && (
                              <button type="button" onClick={(e) => { e.stopPropagation(); setSearchQuery(""); }} className="text-gray-400 hover:text-[#C5A059] p-1 shrink-0">
                                 <X size={14} />
                              </button>
                           )}
                        </form>
                  ...
                  {searchOpen && searchResults.length > 0 && (
                       <div className="absolute top-full left-0 mt-3 w-[320px] bg-white border border-[#C5A059]/30 shadow-xl rounded-sm overflow-hidden z-[120]">
                         <div>
                            {searchResults.map((product) => (
                              <Link 
                                href={`/product/${product.slug}`} 
            ...
            <div className="flex items-center justify-end gap-5 md:gap-6 flex-1">
              <div className="hidden lg:block relative" ref={profileRef}>
                ...
                {/* Profile Dropdown */}
                {profileOpen && session && (
                    <div className="absolute top-full right-0 mt-4 w-60 bg-[#F9F6F0] shadow-2xl border border-[#C5A059]/20 z-[150] rounded-sm overflow-hidden">
              ...
              {navData?.links?.map((link) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`absolute bottom-1 left-1/2 -translate-x-1/2 h-0.5 bg-[#C5A059] transition-all duration-300 ease-out ${isActive ? 'w-full' : 'w-0 group-hover/link:w-2/3'}`} />
                  </div>
                );
              })}
            </div>
        ...
        {/* --- MEGA MENU DROPDOWN (Pure CSS Reveal) --- */}
        {activeCategory && (
            <div 
              className="absolute top-full left-0 w-full bg-[#F9F6F0] border-t border-[#C5A059]/20 border-b shadow-[0_10px_40px_-15px_rgba(0,0,0,0.1)] overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200" 
