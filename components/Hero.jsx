...
const ModernButton = ({ children, link }) => {
  ...
  return (
    <Link href={link || '/product'} className="inline-block relative z-30">
      <button className="group relative px-8 py-4 md:px-10 md:py-4 overflow-hidden transition-all duration-300 hover:w-auto bg-white/10 backdrop-blur-md border border-white/20 hover:bg-[#C5A059] hover:border-[#C5A059]">
...
const Hero = ({ heroData }) => {
  ...
  return (
    // OUTER CONTAINER - Removed margins, paddings, and flex centering to allow full width
    <section className="w-full bg-white relative">
      
      {/* ASPECT RATIO: 
          Mobile: aspect-[4/5] (Tall)
          Desktop: aspect-[21/8] (Cinematic Ultrawide)
      ...
      */}
      <div className="relative w-full aspect-[4/5] md:aspect-[21/8] overflow-hidden bg-neutral-900 isolate touch-pan-y">
                  ...
                  <div className="flex flex-col gap-2 md:gap-4 mb-6 md:mb-8">
                    ...
                    {slide.title && (
                      <motion.h1 
                          ...
                          className="font-heading font-normal text-4xl md:text-6xl lg:text-8xl text-white uppercase tracking-tight leading-[0.9] drop-shadow-lg"
                      >
                          {slide.title}
                      </motion.h1>
                    )}
                    ...
                    <div className="flex flex-col md:flex-row items-start md:items-end justify-between gap-4 md:gap-6 border-t border-white/20 pt-4 md:pt-6">
                        ...
                        {slide.description ? (
                          <motion.p 
                            ...
                            className="text-white/90 text-xs md:text-sm max-w-lg leading-relaxed font-body hidden md:block drop-shadow-md font-medium"
                          >
                             {slide.description}
                          </motion.p>
                        ) : <div />}
        ...
        <div className="absolute bottom-10 right-10 z-30 hidden md:flex gap-2 pointer-events-auto">
            <button 
               onClick={() => paginate(-1)}
               className="w-12 h-12 rounded-sm border border-white/20 bg-black/20 backdrop-blur-md flex items-center justify-center text-white hover:bg-[#C5A059] hover:border-[#C5A059] transition-all duration-300 group shadow-lg"
            >
