...
export async function searchProducts(query) {
  ...
  try {
    // 1. Priority: Matches in Name (Limit 5)
    ...
    // 2. Secondary: Matches in Description (Limit 3, Exclude already found)
    ...
    // Process offers (check if expired)
    const finalResults = await Promise.all(results.map(checkAndResetOffer));
